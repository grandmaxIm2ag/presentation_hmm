  ...
  \scalebox{0.7}{
    \begin{algorithm}[H]
      ...
      Step 1, Initialization :
        $
        \forall i, \alpha_1(i) = \pi_1 b_i(0_1)
        $\\
        Step 2, Induction :\\
        \For{$t\gets2:T$}{
          $
          \alpha_t = \left[ \sum\limits_{j=1}^N\alpha_{t-1}q_{ij}\right]
          b_jO_t
          $
        }
        Step 3, Termination :
        $
        P(0|\lambda) = \sum\limits \alpha_t(i)
        $
    \end{algorithm}
  }
...
\begin{frame}
  ...
  \scalebox{0.6}{
    \begin{algorithm}[H]
      \SetKwInOut{Input}{input }
      \SetKwInOut{Output}{output }
      \Input{$\Lambda$ All the models, $O$ Observed sequence}
